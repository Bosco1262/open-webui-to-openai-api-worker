...
export async function adminChangePassword(
  ...
  // One retry on the epoch bump: the hash write above has already replaced the
  // effective password, so a transient KV failure here must not leave previously
  // issued sessions valid (the bump is what invalidates them). A second failure
  // propagates and the console reports the change as failed — re-running the
  // change flow is always safe (verification uses the new hash).
  //
  ...
  //
  // 纪元自增失败时重试一次：上面的哈希写入已替换生效密码，这里的瞬时 KV 故障不能
  // 让此前签发的会话仍然有效（使它们失效的正是这次自增）。二次失败则向上抛出，
  // 控制台把本次修改报告为失败——重跑修改流程总是安全的（校验用的是新哈希）。
  //
