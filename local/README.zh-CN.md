...
终端全部输出（横幅、日志、错误信息、`--help` 帮助文本）均支持中英文双语，语言选择优先级：

1. `--lang zh` / `--lang en` —— 显式 CLI 参数（优先级最高）；
2. 系统语言检测 —— 环境变量（`LANG` / `LC_ALL` 等）或 Windows 界面语言；
3. 默认英语 —— 检测不到时回退。

`--help` 的输出同样跟随 `--lang`，例如 `python login.py --lang en --help` 始终显示英文帮助。

## 说明

- 凭证通过真实上游鉴权后才算登录成功，避免把"过期 Token 探测请求"或"校园网未认证跳转"误存为有效凭证。校验针对的是**补齐后**的抓取结果（localStorage token + Cookie Jar），也就是最终会落盘的那一份。
- `session.json` 已加入 `.gitignore`，请勿提交到版本库。
- **`session.json` 是一份仍然有效的凭证**（JWT + 会话 Cookie）。导入到 Worker 控制台成功后请立即删除；日后需要新凭证时重新运行本工具即可。一旦它离开你的机器，就应当视为上游会话已泄露，请重新登录以使其轮换。
- POSIX 系统上文件权限会收紧为 `0600`；**Windows** 上文件继承输出目录的 ACL（工具会打印警告）——请保存在私有目录，而不是共享目录。
