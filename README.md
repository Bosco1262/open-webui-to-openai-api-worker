...
- **A client key's privileges are exactly the passthrough allowlist above**: `/models`, `/chat/completions`, `/embeddings` plus `/images`, `/audio`, `/files`. The upstream's account, user, config and chat APIs are unreachable with it (403, and no upstream request is made). Check that this matches your expectations before handing a key to a third party.
